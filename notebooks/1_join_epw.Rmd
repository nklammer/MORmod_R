...
```{r include=FALSE}
library(eplusr)
epw <- eplusr::Epw$new("~/MORmod_R/files/USA_CA_Los.Angeles.Intl.AP.722950_TMY3.epw")
...
load(file = "~/MORmod_R/files/ilas_nodhw.Rda")     # ilas_nodhw
load(file = "~/MORmod_R/files/ilas_dhw_novar.Rda") # ilas_dhw_novar
load(file = "~/MORmod_R/files/hvac_dhw_novar.Rda") # hvac_dhw_novar
load(file = "~/MORmod_R/files/hvac_dhw_var.Rda")   # hvac_dhw_var
...
```{r}
## get total union set of column names
col_names_union <- unique(
c(
  colnames(ilas_nodhw),
...
# every column name in the union set exists in each dataframe?
missing_list <- unique(c(
col_names_union[!(col_names_union %in% colnames(ilas_nodhw))],
col_names_union[!(col_names_union %in% colnames(ilas_dhw_novar))],
col_names_union[!(col_names_union %in% colnames(hvac_dhw_novar))],
col_names_union[!(col_names_union %in% colnames(hvac_dhw_var))]
))



message <- if(length(missing_list)<1) "There is no mismatch" else {
  paste("There is a mismatch.",
        paste(missing_list, collapse = ", "),
        "is present in some but not all of the dataframes")}
```
...
returns_zone_type <- function (zone_string) {
...
## data from Studio Bedroom 1, Third Floor
df_gg <- cbind(db_temp, ilas_nodhw)
ggplot(df_gg, aes(x=db_temp,y=`0_BDRM_1_3`)) + geom_jitter(width = 0.6, height = 0, alpha = 1/20) + geom_smooth(se=F) + theme_classic(base_size = 16) + scale_y_continuous("Ideal Air Loads Cooling (J)") + scale_x_continuous("Drybulb Temperature (°C)", expand = c(0,0), limits = c(4,27)) + ggtitle("Cooling load calculation only,\none apartment unit") + annotate("text", x = 5, y = 2.6*10^6, label = paste("n =",nrow(df_gg)), size = 5) + coord_cartesian(xlim = c(0,30))
```
...
ggplot(df_gg, aes(x=db_temp, y=`0_BDRM_1_3`)) + geom_jitter(width = 0.6, height = 0, alpha = 1/20) + geom_smooth(se=F) + theme_classic(base_size = 16) + scale_y_continuous("Energy Use (J)",labels = scales::scientific_format()) + scale_x_continuous("Drybulb Temperature (°C)", expand = c(0,0), limits = c(5,28)) + ggtitle("All energy end uses,\none apartment unit") + annotate("text", x = 5, y = 1.0*10^7, label = paste("n =",nrow(df_gg)), size = 5) + coord_cartesian(xlim = c(0,30))
```
...
for (col in 1:ncol(x)) { 
  ...
  type <- append(type, rep(returns_zone_type(colnames(x)[col]), times = 8760))
}
...
### export R data of typical weeks for future sampling of EULP

```{r}
typical_extreme_weeks <- epw$typical_extreme_period()
save(typical_extreme_weeks, file = "~/MORmod_R/files/typical_extreme_weeks.Rda")
```
