...
In this case I'm using `hvac_dhw_novar` dataframe to cluster but I'm reducing the  `hvac_dhw_var` dataframe.
...
```{r}
# CHANGE DATA HERE
# the two frames below should have the same dimension
df_cluster <- select(hvac_dhw_novar, -c(CORRIDOR_1, CORRIDOR_2, CORRIDOR_3, CORRIDOR_4))
df_receiver <- select(hvac_dhw_var, -c(CORRIDOR_1, CORRIDOR_2, CORRIDOR_3, CORRIDOR_4))
```
...
time_cols_cluster <- select(df_cluster, c(hour,day,month,hour_index))
time_cols_receiver <- select(df_receiver, c(hour,day,month,hour_index))
...
df_cluster <- select(df_cluster, -c(hour,day,month,hour_index))
df_receiver <- select(df_receiver, -c(hour,day,month,hour_index)) 
...
df_receiver <- cbind(time_cols_receiver,df_receiver)
```
...
if ("minute" %in% colnames(df_cluster)) {
  df_cluster <- df_cluster %>% select(-c(day, hour, month, minute, hour_index))
  df_receiver <- df_receiver %>% select(-c(day, hour, month, minute, hour_index))
} else {
  df_cluster <- df_cluster %>% select(-c(day, hour, month, hour_index))
  df_receiver <- df_receiver %>% select(-c(day, hour, month, hour_index))
}
...
### Save out
compare_C <- red_df
save(compare_C, file = "compare_C.Rda")
```
