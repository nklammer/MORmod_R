...
Bootstrapping dendograms with pvclust. What are *p*-values? "Hierarchical clustering is done for given data and *p*-values are computed for each of the clusters." Function `pvclust()` conducts multiscale bootstrap resampling to calculate *p*-values for each cluster in the result of hierarchical clustering.

```{r}
library(pvclust)
# pvclust clusters columns
# pvclust is bootstrapped
fit <- pvclust(t(df)[1:20, 1:20], method.hclust="ward",
