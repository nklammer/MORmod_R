...
## ESO timeseries from "zemf_ilas_nodhw_eso"

```{r import, message=FALSE, warning=FALSE}
library(readr)
ilas_df <- read_csv("data_in/zemf_ilas_nodhw_eso.csv", 
    skip = 3)
...
#stringr::str_view_all(colnames(ilas_df),"(\\sILA|\\sIdeal|\\sIDEAL).+$")
# delete chars 'ILA*', 'Ideal*', or 'IDEAL*' where * is any character
colnames(ilas_df) <- str_replace_all(colnames(ilas_df),"(\\sILA|\\sIdeal|\\sIDEAL).+$","")
...
```{r}
# drop Date/Time col
time_col <- select(ilas_df, -c(`Date/Time`))
rownames(time_col) <- rownames(ilas_df)
```
...
time_col <- select(ilas_df, c(`Date/Time`))
rownames(time_col) <- rownames(ilas_df)
ilas_df <- select(ilas_df, -c(`Date/Time`)) 
...
area_idx <- sapply(colnames(ilas_df),idx_f) # maps df idx to area idx
area_vec <- area_map$`Space Area [m2]`[area_idx]

# apply normalization vector across columns
ilas_df <- sweep(ilas_df, 2, area_vec, FUN = "/") 
