...
if (downsample == TRUE) {
  if (sample_interval == "day") {
    ...
    df <- cbind(day_idx,df) %>%
      group_by(day_idx) %>%
      summarize(across(-c(day, hour, month, hour_index),sum)) %>%
      select(-c(day_idx))
    hvac_dhw_var_day <- df
  ...
  df <- df %>%
    group_by(month) %>%
    summarize(across(-c(day, hour, hour_index),sum)) %>%
    select(-c(month))
...
if ("hour" %in% colnames(df)) {
  time_cols <- select(df, c(month, day, hour, hour_index))
  rownames(time_cols) <- rownames(df) # need to keep row labels
  df <- select(df, -c(month, day, hour, hour_index)) 
...
maps_name_to_zone <- function(string) { # takes zone string and maps to zone index in area_map
  which(string==area_map$`Zone List`) # returns num vec
  }

area_idx <- sapply(colnames(df), maps_name_to_zone) # maps df idx to area idx
...
for (i in 1:length(APR@clusters)) { # iterate through each cluster
  ...
  area_idx <- sapply(member_zone_names, maps_name_to_zone) # maps cluster idx to area idx
