...
df <- data.frame(temp_db = as.double(db_list), values = as.double(values), zones = as.factor(zones))

```

```{r}
# have to tell aes that the variable is a discrete `factor`
p <- ggplot(df, aes(x = db_list, y = values, color = zones)) + geom_smooth(se = T, span = 0.35, show.legend = F) + xlim(8,25)
...
```{r}
# have to tell aes that the variable is a discrete `factor`
p <- ggplot(df, aes(x = db_list, y = values, color = zones)) + geom_smooth(se = T, span = 0.35, show.legend = F) + xlim(10,21)
