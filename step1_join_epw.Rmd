...
ilas_nodhw_db <- cbind(db_temp,ilas_nodhw)


# how many zone variables?
x <- ilas_nodhw_db %>%
  select(-c(db_temp, day, hour, month)) %>%  select(c(1:10))

num_var <- x %>% length() # 54 zone variables

# create db_temp in length of 54 x 8760
db_list <- rep(ilas_nodhw_db$db_temp, times = num_var)
...
values <- vector(mode = "numeric"); zones <- vector(mode = "character")
for (col in 1:ncol(x)) { 
  values <- append(values, x[,col])
  zones <- append(zones, rep(colnames(x)[col], times = 8760))
}

df <- data.frame(temp_db = as.double(db_list), ilas_nodhw_clg = as.double(values), zones = as.factor(zones))

```

```{r message=FALSE, warning=FALSE}
# have to tell aes that the variable is a discrete `factor`
p <- ggplot(df, aes(x = db_list, y = ilas_nodhw_clg, color = zones)) + geom_smooth(se = T, span = 0.35, show.legend = F) + scale_x_continuous() + coord_cartesian(xlim = quantile(df$db_list,c(0.1,0.9)))
...
```{r include=FALSE}
db_temp <- epw$data()$dry_bulb_temperature # degC
ilas_dhw_novar_db <- cbind(db_temp,ilas_dhw_novar)
...
# create db_temp in length of 54 x 8760
db_list <- rep(ilas_dhw_novar_db$db_temp, times = num_var)
db_list %>% length() == num_var*8760
db_list[1+8760*0] == db_list[1+8760*4]
...
df <- data.frame(temp_db = as.double(db_list), ilas_dhw_novar_clg = as.double(values), zones = as.factor(zones))

```

```{r message=FALSE, warning=FALSE}
# have to tell aes that the variable is a discrete `factor`
p <- ggplot(df, aes(x = db_list, y = ilas_dhw_novar_clg, color = zones)) + geom_smooth(se = T, span = 0.35, show.legend = F) + scale_x_continuous() + coord_cartesian(xlim = quantile(df$db_list,c(0.1,0.9)))
...
df <- data.frame(temp_db = as.double(db_list), hvac_dhw_novar = as.double(values), zones = as.factor(zones))

```

```{r message=FALSE, warning=FALSE}
# have to tell aes that the variable is a discrete `factor`
p <- ggplot(df, aes(x = db_list, y = hvac_dhw_novar, color = zones)) + geom_smooth(se = T, span = 0.35, show.legend = F) + scale_x_continuous() + coord_cartesian(xlim = quantile(df$db_list,c(0.1,0.9)))
...
df <- data.frame(temp_db = as.double(db_list), hvac_dhw_var = as.double(values), zones = as.factor(zones))

```

```{r message=FALSE, warning=FALSE}
# have to tell aes that the variable is a discrete `factor`
p <- ggplot(df, aes(x = db_list, y = hvac_dhw_var, color = zones)) + geom_smooth(se = T, span = 0.35, show.legend = F) + scale_x_continuous() + coord_cartesian(xlim = quantile(df$db_list,c(0.1,0.9)))
